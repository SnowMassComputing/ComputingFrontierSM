...
NOTE -- this section might well move to the next part, where we explicitly have a question about clouds.  Need to see how that evolves a little bit, or perhaps just rephrase here to say that what we call grid computing could someday embrace cloud computing too.
...
HPC facilities have a number of additional attractive features.
...
·      Program managers can allocate resources via the normal allocations process. This would mean holding some resources away from traditional HEP supercomputing efforts. But, if HPC capacity continues to grow at traditional rates while “non-traditional” computing needs grow more slowly, there is an opportunity for significant computing cycles to be available.
·      Programmatic funds from HEP could be contributed directly to HPC centers to support HEP efforts (e.g. from DOE HEP to NERSC or indirectly through ASCR).
·      Support for hardware procurement, operation, and support at one of more HPC centers can be contributed directly by research teams or facilities.
...
processors. If the HEP community does not adapt codes to run in parallel (using fine-grained parallelism [ e.g. GPUs] and possibly coarse-grained parallelism [e.g. MPI]) it will be precluding itself from using a vast source of computational power in a time of greater need.
 
[Coordination: software infrastructure and identify management] Anything else?
ection{Summary}
