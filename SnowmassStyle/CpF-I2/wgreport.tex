...
\textcolor{red}{I think we need to make a stronger distinction between the HPC centers and the HPC computing paradigm here.  We're talking here about the possibility of IF experiments using the centers, not the HPC paradigm.  Maybe this section can be an opportunity to talk about the centers' interest into supporting computing that doesn't fit the traditional HPC paradigm?}  Intensity Frontier experiments have combined computing requirements on a scale that could map onto the HPC center model. The experiments have a diverse set of needs, but a survey found that there is significant commonality among them. Some experiments have already made effective use of HPC centers' facilities and expertise. For example, the Daya Bay experiment analysis was conducted at NERSC, which also served as the Tier 1 data center.  NERSC also has expertise gained from supporting KAMLAND, IceCube, BaBar, SNO, ALICE, ATLAS, and Palomar PTF data analysis.  There may be an opportunity for Intensity Frontier experiment to band together and take advantage of centralized facilities as a source of computing and storage and a gathering place for software repositories and expertise on common requirements.
...
increases (cite NERSC report) \textcolor{red}{Is this HPC paradigm or HPC center resources?}. 
...
From the standpoint of the HPC centers, there are challenges to accommodating an expanded workload. The addition of traditional HTC workflows, and those currently using the WLCG in particular, onto national HPC centers will require integrating them into job scheduling systems already in place. Serial jobs, which characterize most of WLCG computing, is not a good fit with large parallel computational systems. Identity management and data ownership and management pose other obstacles. Finally, providing computing, storage, and services to a new community requires an allocation of resources from funding agencies either through normal allocations processes or through outside funding from the community to augment the HPC centers’ base program.
 
The good news is that at least one HPC Center -- NERSC -- has a long history of supporting “non-traditional” HEP efforts.  NERSC hosts the PDSF cluster (which has its origins in the Superconducting Supercollider), which is the Tier 1 site for the Daya Bay neutrino experiment and a Tier 3 site for ATLAS. PDSF is also used by ALICE, [update from Craig]. NERSC also hosts the Lattice Connection and Deep Sky web data-driven science gateways for LQCD field configurations [check] and supernova discovery. NERSC’s HPC systems were used to calculate the more than 1 million [check] simulated realizations of the early universe needed to process results from the Planck CMB satellite mission. NERSC computers were also used to calculate synthetic supernova spectra that played a key role in the discovery of ``dark energy'' by Perlmutter {\it et al}.  NERSC has the flexibility , open access, close affiliation with sister center ESnet, and know-how to support “non-traditional” HPC if funding is available to support it.
 Providing computing, storage, and services to a new community requires an allocation of resources from funding agencies either through normal allocations processes or through outside funding from the community. There are a number of potentially feasible ways to accommodate an influx of “non-traditional” (e.g. WLCG work) into national HPC centers:

\begin{itemize} 
\item Program managers can allocate resources via the normal allocations process. This would mean holding some resources away from traditional HEP supercomputing efforts. But, if HPC capacity continues to grow at traditional rates while “non-traditional” computing needs grow more slowly, there is an opportunity for significant computing cycles to be available.
\item Programmatic funds from HEP could be contributed directly to HPC centers to support HEP efforts (e.g. from DOE HEP to NERSC or indirectly through ASCR).
\item Support for hardware procurement, operation, and support at one of more HPC centers can be contributed directly by research teams or facilities.
\end{itemize} 

There are a number of examples of the latter model at NERSC, where the Berkeley HEP and Nuclear Physics community funds the PDSF cluster and the Joint Genome Institute is now using NERSC to host its computing, storage, and support needs. NERSC also administers resources funded by the Planck Satellite mission and the Palomar Transient Sky Survey. Discussions are currently underway for similar projects with the Berkley Lab Advanced Light Source, the LCLS at Stanford, the DOE KBase project, and the Large Synopic Sky Telescope project.
 

\subsection{HPC Center Growth Rates and Plans}
 
As shown in Figure \ref{fig:NERSC-Computational-Hours} above the projected need for HEP production computing within DOE will outpace even the traditional Moore's Law increase of approximately 2X per year.  The red line in the plot connects the stated requirements with 2012 usage and shows that just the traditional HPC HEP computing community has needs that far exceed the traditional trend lines. Much of this is driven by the need for simulations to support interpretation of data being collected from the LHC and cosmological sky surveys. \textcolor{red}{I thought that a lot of what is driving this is lattice QCD, which is not what is limiting LHC interpretation -- we're in the perturbative world.  Intensity frontier, maybe so.}  Additional computing is needed for accelerator research and design.
 
This pressing need for resources comes at a time of uncertain budgetary forecasts. The DOE exascale initiative targets enabling an exascale compute system by 2022-2023 -- with the actual timeframe dependent on funding and technology advances -- through the Fast Forward (processor design) and Design Forward (system design) partnerships with HPC vendors. The Exascale Initiative includes partnerships with government, the computer industry, DOE laboratories, and academia.  Goals include
\begin{itemize}
...
[insert discussion/plot of NSF and DOE LCF capabilities and plans. LCF futures? LCF resources are not programmatic production cycles with guaranteed access. Even less good fit than NERSC.]
 
 
While the past focus on HPC centers has largely been on massive parallel computations, the need for, and ability to facilitate handling of, accompanying data has quietly grown over the years. The plot below shows that archival data at NERSC has grown consistently at about 1.7X per year. Figure~\ref{fig:NERSC-HPSS} shows the historical growth in data stored at NERSC's archival storage systems and extrapolation of that trend. The findings from the NERSC HEP requirements reviews show a sharp increase above the trend line starting in 2013-2014 through 2017. This largely reflects a need to support simulated and experimental data from sky surveys, including LSST and Cosmic Background Radiation satellite missions, as well a growing need to centrally store and analyze experimental data from satellites and accelerators.

\begin{figure}[h]
\includegraphics[width=\textwidth]{CpF-I2/images/2013-NERSC-HPSS-Usage.png}
\caption{There is now wide recognition within the HPC community that “Big Data” is a growing and crucial component of HPC.  [Insert info from DOE/ASCR/HEP Data Summit]}
\label{fig:NERSC-HPSS}
\end{figure}
...
We are at a critical time for HPC computing. To continue the historic, Moore's Law-like, growth in HPC computing power, the HPC community must adapt to build (with HPC vendors) and use new energy-efficient technologies. The path forward will require use of low-power, simple fundamental processing units. The current leading architectural candidates are systems that incorporate NVIDIA GPUs (Graphical Processing Units), the Intel Phi “many-core” processor, processors based on AMD's APU technology, and units integrating ARM-based processors.
 
In addition to engineering and technology advancements needed to actually build  energy-efficient, performant processors and system, application development on these new architectures poses a serious challenge. The programming strategy, in practice since the late 1990s, of optimizing code to minimize CPU cycles and inter-node communication must be replaced with a paradigm that stresses minimizing data movement at all levels (from on-chip cache, to on-node shared memory, to storage media) and maximizing SIMD (vector) processing operating on contiguous data, and use of light-weight threads of execution. Virtually all HPC software will have to be modified significantly or rewritten to execute acceptably well (or at all in some cases) on these architectures. Programming models for their architectures are nascent and/or proprietary to a particular vendor’s architecture. Some science communities have embraced one technology or another, while others are waiting to see which programming model and/or language “wins out.” In either case, a significant investment in code development will be required.
 
This issue also applies to HTC applications, like those that currently use the WLCG. Although traditional multi-core processors and systems will continue to exist for some time, the growth in computing power will come from “many-core”
processors. If the HEP community does not adapt codes to run in parallel (using fine-grained parallelism ({\it e.g.} GPUs) and possibly coarse-grained parallelism ({\it e.g.} MPI)) it will be precluding itself from using a vast source of computational power in a time of greater need.
...
Cosmic-frontier experiments and the simulations needed to interpret them are among the drivers of a need for growth in HPC resources in the coming years, along with lattice QCD and accelerator design.  Indeed, demand for access to HPC across HEP is expected to exceed the expected availability.  Such computations is critically needed to interpret results from a number of important experiments and realize scientific returns from national investments in those experiments.  The NERSC report on HEP computing needs indicates a shortage of HPC resources needed for HEP by a factor of four by 2017.  While funding and technology development needed to sustain traditional HPC growth rates are uncertain, they must be maintained to support HEP science.
...
National centers play an important role in some aspects of computing, and HEP might be able to take advantage of an expanded role.  It is already used in many of the applications listed above.  While there are not enough resources available at the centers to rival those of the WLCG, experiments should explore the use of the centers as part of their efforts to diversify their computing architectures.  These centers do have access to large, state-of-the-art resources, operational support and expertise in many areas of computing.
...
\bibitem{bib:CHEPresources} See for instance G.~Bagliesi {\it et al.}, ``CMS resource utilization and limitations on the grid after the first two years of LHC collisions'', CHEP 2012 proceedings, {\tt http://dx.doi.org/10.1088/1742-6596/396/3/032012}.
\end{thebibliography}
